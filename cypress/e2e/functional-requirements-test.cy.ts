...
//   })
  ...
  it('The user must be able to see a list of their clips they wish to add to the editor timeline', () => {
    visitHomeAndAddURL()
    const terminput = visitTermsInputAndAdd()
    visitCaptionsPageAndClickClip(terminput)
    //go to download page and add clip
    cy.visit('http://localhost:3000/download')
    cy.get('[data-testid=CancelIcon]', {timeout: 80000}).click()
    cy.visit('http://localhost:3000/editor')
    cy.get('[data-testid=add-to-timeline-btn').click({multiple: true})
  })
})
